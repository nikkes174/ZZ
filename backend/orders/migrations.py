...
async def ensure_order_bonus_columns() -> None:
    ...
    statements = [
        ...
        "ALTER TABLE orders ADD COLUMN IF NOT EXISTS iiko_request_order_id VARCHAR(64)",
        "ALTER TABLE orders ADD COLUMN IF NOT EXISTS iiko_pos_order_id VARCHAR(64)",
        ...
        "ALTER TABLE orders ADD COLUMN IF NOT EXISTS iiko_error_code VARCHAR(128)",
        "ALTER TABLE orders ADD COLUMN IF NOT EXISTS iiko_error_message TEXT",
        "ALTER TABLE orders ADD COLUMN IF NOT EXISTS iiko_last_error_at TIMESTAMPTZ",
        "ALTER TABLE orders ADD COLUMN IF NOT EXISTS iiko_recovery_checks INTEGER NOT NULL DEFAULT 0",
        "UPDATE orders SET iiko_request_order_id = iiko_order_id WHERE iiko_request_order_id IS NULL AND iiko_order_id IS NOT NULL",
        "CREATE UNIQUE INDEX IF NOT EXISTS ix_orders_iiko_request_order_id ON orders (iiko_request_order_id) WHERE iiko_request_order_id IS NOT NULL",
        "CREATE INDEX IF NOT EXISTS ix_orders_iiko_pos_order_id ON orders (iiko_pos_order_id) WHERE iiko_pos_order_id IS NOT NULL",
